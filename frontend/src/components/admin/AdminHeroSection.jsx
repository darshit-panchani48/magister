// src/components/admin/AdminHeroSection.jsx — Original Desktop Layout + Mobile-Only Box Theme

import React from 'react';
import { format } from 'date-fns';
import collegePhoto from '../../assets/college-photo.avif';

export default function AdminHeroSection({ adminAppId, profile, onEditProfile }) {
  ...
  return (
    <section className="admin-hero-outer-section">
      <div className="admin-hero-card-box">
        {/* Avatar Area */}
        <div className="admin-avatar-box">
          ...
          >
            {profile?.photo?.url ? (
              ...
              <i className="ti ti-shield" style={{ fontSize: 28 }} />
            )}
          </div>
        </div>
          ...
          {isComplete ? (
            <>
              ...
              <div className="admin-details-grid">
                {[
                  { icon: 'ti-id-badge', label: 'APP ID', value: adminAppId },
                  ...
                  { icon: 'ti-text-caption', label: 'Bio', value: profile.bio },
                ]
                  .filter((r) => r.value)
                  .map((row) => (
                    <div key={row.label} className="admin-detail-item">
                      <i className={`ti ${row.icon} admin-detail-icon`} />
                      <div className="admin-detail-text">
                        <div className="admin-detail-label">{row.label}</div>
                        <div className="admin-detail-value">{row.value}</div>
                      </div>
                    </div>
                  ))}
              </div>
              ...
              <div className="admin-btn-row">
                <button onClick={onEditProfile} className="admin-edit-btn">
            ...
            <div>
              ...
              <div style={{ fontSize: 13, color: '#6b7280', marginBottom: 14 }}>
                Complete your profile to personalize the admin panel.
              </div>
              {adminAppId && (
                <div style={{ fontSize: 12, color: '#9ca3af', marginBottom: 14 }}>
                  Logged in as: <strong style={{ color: '#374151' }}>{adminAppId}</strong>
                </div>
              )}
              <button onClick={onEditProfile} className="admin-complete-btn">
        ...
        {/* Campus Photo */}
        <div className="admin-campus-img">
          <img
            src={collegePhoto}
            alt="ASSC Campus"
